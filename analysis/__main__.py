...
from .visualizations import generate_all as generate_figures
...
def _model_count_table(desc: dict[str, Any]) -> Table:
...
def _matrix_table(rho_df: pd.DataFrame, p_adj_df: pd.DataFrame) -> Table:
    rho_arr: np.ndarray[tuple[int, int], np.dtype[np.floating[Any]]] = rho_df.to_numpy()
    p_arr: np.ndarray[tuple[int, int], np.dtype[np.floating[Any]]] = p_adj_df.to_numpy()
    tbl = Table("", *CODE_DIMS, box=None, padding=(0, 2))
    for i, pdim in enumerate(PROMPT_DIMS):
        cells = [
            f"[{_rho_color(float(rho_arr[i, j]))}]{rho_arr[i, j]:.3f}[/][dim]{sig_stars(float(p_arr[i, j]))}[/]"
            for j in range(len(CODE_DIMS))
        ]
...
def _model_corr_table(model_corr: list[ModelCorr]) -> Table:
    ...
    for row in model_corr:
        tbl.add_row(
            ...
            f"[{_p_color(row['p_value'])}]{_fmt_p(row['p_value'])}{sig_stars(row['p_value'])}[/]",
        )
    return tbl


def _kw_table(kw: dict[str, dict[str, float]]) -> Table:
...
def _syntax_table(syn_rho: pd.DataFrame) -> Table:
    syn_arr: np.ndarray[tuple[int, int], np.dtype[np.floating[Any]]] = syn_rho.to_numpy()
    col_names = [str(c).removeprefix("ruff_") for c in syn_rho.columns]
    tbl = Table("", *col_names, box=None, padding=(0, 2))
    for i, cdim in enumerate(CODE_DIMS):
        cells = [f"[{_rho_color(float(syn_arr[i, j]))}]{syn_arr[i, j]:.3f}[/]" for j in range(syn_arr.shape[1])]
...
def main() -> None:
    ...
    cout("\nSpearman ρ (prompt → code, Bonferroni-adjusted):")
    cout(
    ...
    cout("\nPrompt dim ranking (mean ρ across code dims):")
    cout(
    ...
    cout("\nSyntax vs semantic ρ (code dims vs static metrics):")
    cout(
    ...
    generate_figures(df, rho_df, p_adj_df, reg, model_corr, syn_rho)
